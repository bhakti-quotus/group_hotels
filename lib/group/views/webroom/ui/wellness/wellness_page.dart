...
class _WellnessBody extends StatelessWidget {
  const _WellnessBody();

  @override
  Widget build(BuildContext context) {
    final ctrl = Get.find<WellnessController>();

    return Column(
      children: [
        _SearchBar(ctrl: ctrl),
        _CategoryChips(ctrl: ctrl),
        Expanded(child: _WellnessList(ctrl: ctrl)),
      ],
    );
  }
}
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search a service or an activity',
...
class _WellnessList extends StatelessWidget {
  final WellnessController ctrl;
  const _WellnessList({required this.ctrl});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      return ListView.builder(
        ...
        itemBuilder: (_, i) => _WellnessCard(item: items[i]),
      );
    });
  }
}

/// Small badge showing how many images the item has (only shown if > 1).
class _ImageCountBadge extends StatelessWidget {
  final Map<String, dynamic> item;
  const _ImageCountBadge({required this.item});

  @override
  Widget build(BuildContext context) {
    final images = item['images'] as List<dynamic>?;
    final count = images?.length ?? 0;
...
class _WellnessCardState extends State<_WellnessCard> {
  ...
  List<String> get _images {
    ...
    // Fallback: support legacy single 'image' field
        ...
        children: [
          ...
          Stack(
            children: [
              ...
              // Image counter badge (top-right, below close button) — only shown when > 1 image
              if (images.length > 1)
                Positioned(
                  top: 54,
                  right: 14,
              ...
              // Left arrow (shown when not on first image)
              ...
              // Right arrow (shown when not on last image)
...
void _showFilterSheet(BuildContext context, WellnessController ctrl) {
  showModalBottomSheet(
    context: context,
    isScrollControlled: true,
